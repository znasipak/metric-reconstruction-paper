...
color_list = mpl.cm.viridis(np.linspace(0, 0.98, max_pn_order + 1))
...
for i in range(max_pn_order + 1):
    ...
    ax[0].errorbar(u1_num_data[p_idx_start:, e_idx, 0] - 2, data_comp, yerr=err_comp, label=f"{i}PN", fmt = markers[i], capsize = 3, color = color_list[i], markerfacecolor = "white", rasterized = True)


    e_idx = 3
    data_comp = np.abs(u1_pn_data[i, p_idx_start:, e_idx, 3] - u1_num_data[p_idx_start:, e_idx, 2]) / np.abs(u1_num_data[p_idx_start:, e_idx, 2])
    err_comp = np.abs( u1_pn_data[i, p_idx_start:, e_idx, 3] / u1_num_data[p_idx_start:, e_idx, 2] ** 2 ) * u1_num_data[p_idx_start:, e_idx, 3]
    ax[1].errorbar(u1_num_data[p_idx_start:, e_idx, 0] - 2, data_comp, yerr=err_comp, label=f"{i}PN", fmt = markers[i], capsize = 3, color = color_list[i], markerfacecolor = "white", rasterized = True)
