...
if __name__ == "__main__":
    ...
    markers = ['o', 'x', 'd']
    fig, ax = plt.subplots(2, 3, figsize=(10.5, 4), sharex=True, sharey=True)
    ...
    for comp_iter_test in range(0, 1):
    ...
    for comp_iter_test in range(0, 1):
        ...
        if comp_iter_test == 0: # include complex conjugate
            ...
            data_outer = 2*np.real(data_outer)
        ymax = 1.1*np.max([np.abs(data_inner[-1]), np.abs(data_outer[-1])])
        ax[offset+comp_iter_test, 0].plot(lmodes, np.abs(data_inner), markers[0], ms = 3, fillstyle='none', label="$r \\rightarrow r_p^-$")
        ax[offset+comp_iter_test, 0].plot(lmodes, np.abs(data_outer), markers[1], ms = 3, fillstyle='none', label="$r \\rightarrow r_p^+$")
        ax[offset+comp_iter_test, 0].plot(lmodes, 0.5*np.abs(data_inner+data_outer), markers[2], ms = 3, fillstyle='none', label="average")
        ax[offset+comp_iter_test, 0].plot(lmodes, ymax*(lmodes/lmax)**(0)/1.1, '--', color='gray', lw=0.5)
        ax[offset+comp_iter_test, 0].set_yscale('log')
        ax[offset+comp_iter_test, 0].set_ylabel(labels[comp_iter_test])
    ...
    for comp_iter_test in range(0, 1):
        ...
        ymax = 1.1*np.max([np.abs(data_inner[-1]), np.abs(data_outer[-1])])
        ax[offset+comp_iter_test, 1].plot(lmodes, np.abs(data_inner), markers[0], ms = 3, fillstyle='none', label="$r \\rightarrow r_p^-$")
        ax[offset+comp_iter_test, 1].plot(lmodes, np.abs(data_outer), markers[1], ms = 3, fillstyle='none', label="$r \\rightarrow r_p^+$")
        ax[offset+comp_iter_test, 1].plot(lmodes, 0.5*np.abs(data_inner+data_outer), markers[2], ms = 3, fillstyle='none', label="average")
        ax[offset+comp_iter_test, 1].plot(lmodes, ymax*(lmodes/lmax)**(0)/1.1, '--', color='gray', lw=0.5)
        ax[offset+comp_iter_test, 1].set_yscale('log')
        ax[offset+comp_iter_test, 1].set_ylabel(labels[comp_iter_test])
    ...
    for comp_iter_test in range(0, 1):
        ...
        ymax = 1.1*np.max([np.abs(data_inner[-1]), np.abs(data_outer[-1])])
        ax[offset+comp_iter_test, 2].plot(lmodes, np.abs(data_inner), markers[0], ms = 3, fillstyle='none', label="$r \\rightarrow r_p^-$")
        ax[offset+comp_iter_test, 2].plot(lmodes, np.abs(data_outer), markers[1], ms = 3, fillstyle='none', label="$r \\rightarrow r_p^+$")
        ax[offset+comp_iter_test, 2].plot(lmodes, 0.5*np.abs(data_inner+data_outer), markers[2], ms = 3, fillstyle='none', label="average")
        ax[offset+comp_iter_test, 2].plot(lmodes, ymax*(lmodes/lmax)**(3)/1.1, '--', color='gray', lw=0.5)
        ax[offset+comp_iter_test, 2].plot(lmodes, 0.5*np.abs(data_inner[-1]+data_outer[-1])*(lmodes/lmax)**(2), '--', color='gray', lw=0.5)
        ax[offset+comp_iter_test, 2].set_yscale('log')
        ax[offset+comp_iter_test, 2].set_xscale('log')
        ax[offset+comp_iter_test, 2].set_xticks([2, 5, 10, 20, 35])
        ax[offset+comp_iter_test, 2].set_xticklabels([2, 5, 10, 20, 35])
        ax[offset+comp_iter_test, 2].set_ylabel(labels[comp_iter_test])

    ax[-1, 0].set_xlabel("$\\ell$")
    ax[-1, 1].set_xlabel("$\\ell$")
    ax[-1, 2].set_xlabel("$\\ell$")

    ax[0, 0].legend(loc='upper left', fontsize=10, ncol=2, frameon=True, reverse=False, alignment='right')
    plt.savefig(f"figures/hretlm_a{spin}_p{p0}_singular_lmode_convergence_reduced.pdf", bbox_inches='tight')