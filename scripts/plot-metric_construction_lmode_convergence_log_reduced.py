...
if __name__ == "__main__":
    ...
    p0_vals = np.concatenate((p0_vals[0:2], p0_vals[-1:]))
    ...
    for k in range(kcells):
        ...
        for i in [0,1]:
            ...
            for ll in lrange_list:
                ...
                ax[k, 1].text(2*(p0 - rlower1), 1e-2, f"$p = {int(p0)}$", fontsize=14, color = 'grey', usetex = True)

                ax[k, 0].set_ylabel(f"$\\hat{{\\Delta}}^{{\\mathrm{{{gauge}}},\\ell_\\mathrm{{max}}}}_{{lm}}$")
                j += 1
    ax[-1, 0].legend(loc='upper left', fontsize=11, ncol = 1, frameon=False)
    ax[0,0].set_title('$a = 0.6$', fontsize=fs)
    ax[0,1].set_title('$a = 0.9$', fontsize=fs)
    ax[-1, 0].set_xlabel('$r/M$')
    ax[-1, 1].set_xlabel('$r/M$')
    plt.savefig(f"figures/lmode-contribution-log-{gauge}-reduced.pdf", dpi=300, bbox_inches='tight', format='pdf')